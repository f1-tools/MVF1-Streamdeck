...
@action({ UUID: "com.f1-tools.mvf1.forward" })
export class Forward extends SingletonAction<Settings> {
    ...
    override async onKeyDown(ev: KeyDownEvent<Settings>): Promise<void> {
        ...
        if (settings.global === "AP") {
            ...
            this.openPlayerSelectorForForward(ev);
        }
    }
    ...
    override async onDidReceiveSettings(ev: DidReceiveSettingsEvent<Settings>): Promise<void> {
        if (ev.payload.settings.nSeconds) {
            Forward.nSeconds = ev.payload.settings.nSeconds;
        }
    }
     ...
     */
    private async globalForward(nSeconds: number) {
        this.pressed.value = true;
     ...
     */
    private async openPlayerSelectorForForward(ev: KeyDownEvent<Settings>) {
        const newGlobalSettings: GlobalSettings = {
            playerPickerCaller: PlayerPickerCaller.FORWARD
        };
     ...
     */
    public static async playerSelectedForOneTimeForward(playerId: string) {
        seekPlayerBySeconds(playerId, Forward.nSeconds ?? 10);
    }
}